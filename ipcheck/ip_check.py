...
from ipcheck.app.utils import is_ip_address, is_ip_network, gen_time_desc, parse_url, is_hostname, copy_file, print_file_content, console_print
...
def check_or_gen_def_config(def_cfg_path):
    if not os.path.exists(def_cfg_path):
        ...
        console_print('配置文件已生成位于 {}'.format(def_cfg_path))
...
def load_config():
    ...
    console_print('当前配置文件为:', config_path)
    Config.CONFIG_PATH = config_path
    config = Config()
    config.pure_mode = args.pure_mode
    console_print('纯净模式:', config.pure_mode)
    config.ro_verbose = args.verbose
    console_print('是否开启调试信息:', config.ro_verbose)
    ...
    console_print('测试源文件为:', config.ro_ip_source)
    ...
    if config.ro_prefer_ports:
        console_print('ip:port 测试源端口为:', config.ro_prefer_ports)
    ...
    console_print('期望平均网速为: {} kB/s'.format(config.st_avg_download_speed))

    # 检查快速测速开关
    if args.fast_check:
        config.st_fast_check = True
    if config.st_fast_check:
        console_print('快速测速已开启')
    ...
    console_print('优选ip 文件为:', config.ro_out_file)
    config.no_save = args.dry_run if args.dry_run else args.no_save
    console_print('是否忽略保存测速结果到文件:', config.no_save)
    # 处理限制参数
...
def write_better_ips_to_file(ips: Iterable[IpInfo], path):
    with open(path, 'w', encoding='utf-8') as f:
        ...
        f.write('\n')
    console_print('测试通过%d个优选ip 已导出到' % len(ips), path)
...
def run_ip_check():
    ...
    if not ip_list_size:
        console_print('没有从参数中生产待测试ip 列表, 请检查参数!')
        os._exit(0)
    else:
        console_print('从参数中生成了{} 个待测试ip'.format(ip_list_size))

    # 可用性测试
    valid_test_config = config.get_valid_test_config()
    if config.ro_verbose:
        console_print('\n可用性测试配置为:')
        console_print(valid_test_config)
    ...
    if passed_ips:
        ...
        console_print('下载测试没有获取到可用ip, 测试停止!')
        return
...
def config_edit():
    ...
    if args.example:
        ...
        return
    console_print('编辑配置文件 {}'.format(config_path))
    ...
    if platform.startswith('win'):
        ...
        console_print('未知的操作系统, 请尝试手动修改{}!'.format(config_path))
