...
from ipcheck.app.utils import adjust_list_by_size, get_perfcounter, show_freshable_content
...
class RttTest:
    ...
    def __test(self, ip_info: IpInfo) -> IpInfo:
        if StateMachine().ipcheck_stage != IpcheckStage.RTT_TEST:
            return ip_info
        return asyncio.run(async_tcpping(ip_info,
...
async def async_tcpping(
    ...
    if packets_sent == 0:
        return ipinfo
    ipinfo.rtt = round(sum(rtts) / packets_sent, 2)
